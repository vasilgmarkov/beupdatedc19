<template>
  <div>
    <div>
     <marquee behavior="scroll" direction="left"  v-if="globalStats">
...
console.log(today)
      today.data.forEach((country, i) => {
        if (
          ...
          this.coutryCodes.hasOwnProperty(country.countrycode.toLowerCase())
        ) {
          ...
          var myIcon = L.divIcon({
            ...
            html: `<div style='background-color:#4838cc;' class='marker-pin'></div><img style='z-index:3;position: absolute;width: 24px;border-radius: 40px;bottom: 25%;left: 10%;' src='https://www.countryflags.io/${country.countrycode.toLowerCase()}/flat/32.png'> `,
            iconSize: [30, 42],
            iconAnchor: [15, 42],
            popupAnchor: [-3, -76],
          ...
          //     target="_self"
          //     onclick="event.preventDefault(); ${router.push(`'/country/${country.countrycode}'`)};"
          //   >${country.countrylabel }</a>

          // <br>${country.totalcases}<br>${today.date}` );

          marker.bindPopup(`
  <h4>${country.countrylabel}</h4>
  <h6>№ Cases</h6>
  ...
  <p>😓 ${country.totaldeaths}</p>
    <a
      href="/country/${country.countrycode}"
      target="_self"
      onclick="event.preventDefault(); Vue.router.push('/country/${country.countrycode}');"
    >More info</a>
`);
